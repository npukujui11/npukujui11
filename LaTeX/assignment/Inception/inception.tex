...
%	\end{frame}
	...
	\vspace{0.3cm}
		...
		{\zihao{-5} \yahei \textbf{弱光图像增强}旨在从黑暗和噪声图像中恢复正常光线和无噪声图像，这是一个长期存在的重要计算机视觉课题。在硬件条件不变的情况下，给定降质图像或恶劣天气下拍摄的图像，研究生成高质量、无噪声图像等增强方法，在实际应用中非常有意义。}
	...
	\begin{frame}
		...
		{\zihao{-5} \yahei 基于软件技术的图像复原增强技术得到了广泛研究，其通过引入外部信息、加入先验、基于大数据集进行学习等方式，将输入的弱光图像的低光细节复原，产生接近降质前理想质量的低噪图。}
...
%	\end{frame}
	...
	\begin{frame}
		...
		\begin{itemize} 
			...
			% 近年来该领域的研究进展主要集中在亮度的细化上，而忽略了色度的作用。
			
			% 目前为止传统方法和基于学习的方法大多主要着眼于提高亮度的质量，这容易导致严重的色度信息丢失和不愉快的视觉效果。
			
			\begin{itemize}
			
			\item BCNet (Brihten-and-Colorize Network)	
				
			% 作者提出了一个“亮度和着色”网络，它将图像着色引入 LLIE，以解决上述问题。
			
			% BCNet 可以实现精确色彩的 LLIE，同时可以根据用户喜好定制不同饱和度和色彩风格的增强。
			
			% 具体来说，BCNet 将 LLIE 视为一个多任务学习问题:增亮和着色。增亮子任务与其他传统的 LLIE 方法一致，以获得良好的亮度。着色子任务是通过将弱光图像的色度作为颜色指导来完成的，就像用户指导图像着色一样。
			
				\begin{itemize}
					
					\item Multitasking-learning Architecture 
					
					% BCNet 采用多任务学习架构，该架构包含一个编码器和两个特定任务的解码器。编码器以弱光亮度作为输入，而两个特定任务的解码器分别输出预测亮度和色度。通过同时训练两个子任务，增亮子任务可以为着色子任务提供正常亮度信息。此外，引入颜色分类损失，将着色转化为分类任务，缓解着色子任务中的颜色消失。
					
					% 特点：多任务学习架构通过在相关任务之间共享知识，旨在提高学习效率和模型的泛化能力。
			...
			% 在完成模型训练后，用户可以简单地操纵颜色指导(即输入的低光色度)，以获得定制的结果。
				...
				\begin{itemize}
					...
					% 色度是指颜色的纯度，它描述了颜色中纯色的强度相对于灰色的程度。色度是一个相对于白色背景的概念，它衡量的是颜色与白色之间的差异程度。
					
					% 饱和度也是描述颜色纯度的一个度量，但它是相对于颜色的最大纯度版本的。
					% 饱和度是一个相对于颜色自身最鲜艳状态的概念，它衡量的是颜色的强度或深度。
					\item Local Consistency
					% 有方法利用 HSV 色彩空间中的直方图实现了色彩定制，而直方图作为全局信息很难保证局部一致性。
					% 局部一致性指在一幅图像中，相邻像素通常具有相似的颜色和亮度，这种现象就是局部一致性的体现。这一概念在边缘检测、纹理分析、图像分割等任务中非常重要。
				\end{itemize}
