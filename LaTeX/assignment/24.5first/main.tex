...
%		\end{itemize}
	...
	\section{实验进度}
		...
		我们尝试了不同的 Encoder 和 Decoder 组合，如 Fig. \ref{fig: Encoder and Decoder} 所示，对于 Encoder 部分，我们首先应用注意力机制（Attention），然后使用 $3 \times 3$ 的轴向深度可分离卷积 (Depthwise Separable Convolution)，接着进行批标准化 (Batch Normalization) 和 $1 \times 1$ 卷积操作，最后通过最大池化 (MaxPooling) 进行特征压缩，最终使用 Sigmoid 激活函数。对于 Decoder 部分，我们首先将其与对应的 Encoder 的跳跃连接进行 Concat 操作，然后进行上采样 (Upsample) 操作，接着应用注意力机制 (Attention) ，再次使用 $3 \times 3$ 的轴向深度可分离卷积，再通过批标准化和 $1 \times 1$ 卷积操作，最终采用 Sigmoid 激活函数。
		
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.8\linewidth]{picture/LLIE/Experiment/Encoder and Decoder}
			%\captionsetup{font=scriptsize}
			\caption{Encoder 和 Decoder 的搭配}
			\label{fig: Encoder and Decoder}
		\end{figure}
		
		我们使用 MBLLEN 中的 EM 模块，旨在加强每个层级中Encoder和Decoder之间的特征连接，从而增强模型的表现能力，如 Fig .\ref{fig: U-Net Architecture}所示。
		
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.8\linewidth]{picture/LLIE/Experiment/U-Net Architecture}
			%\captionsetup{font=scriptsize}
			\caption{加入 EM 模块}
			\label{fig: U-Net Architecture}
		\end{figure}
		
		其中呈现的效果如 Fig. \ref{fig: Visualization Results}，
		我们分别进行了下列 6 种尝试：
		\begin{itemize}
			\item[(1)] 我们使用 3 层 UNet 结构(3个 Encoder 和 Decoder)，在 LOLv2 数据集下，模型训练的可视化结果，如 Fig. \ref{fig: ULite_Enhance_3_layer} 所示。 
			\item[(2)] 我们使用 3 层 UNet 结构，并在同层级的 Encoder 和 Decoder 中添加 EM 模块，在 LOLv2 数据集下，模型训练的可视化结果，如 Fig. \ref{fig: ULite_Enhance_3_layer_EM} 所示。
			\item[(3)] 我们使用 3 层 UNet 结构，添加 EM 模块，此外我们去除掉 BottleNeck 结构，并将最底层的 Encoder 和 Decoder 通道修改为 512，此外我们将激活函数从原先的 Sigmoid 设置为 ReLU，在 LOLv2 数据集下，模型训练的可视化结果，如 Fig. \ref{fig: ULite_Enhance_3_layer_EM_withou_Transformer_maxc_512_ReLU} 所示。
			\item[(4)] 我们使用 3 层 UNet 结构，添加 EM 模块，此外我们去除掉 BottleNeck 结构，并将最底层的 Encoder 和 Decoder 通道修改为 256，在 LOLv2 数据集下，模型训练的可视化结果，如 Fig. \ref{fig: ULite_Enhance_3_layer_EM_without_Transformer} 所示。
			\item[(5)] 我们使用 3 层 UNet 结构，添加 EM 模块，此外我们去除掉 BottleNeck 结构，并将最底层的 Encoder 和 Decoder 通道修改为 512，在 LOLv2 数据集下，模型训练的可视化结果，如 Fig. \ref{fig: ULite_Enhance_3_layer_EM_without_Transformer_maxc_512} 所示。
			\item[(6)] 我们使用 4 层 UNet 结构，添加 EM 模块，此外我们去除掉 BottleNeck 结构，在 LOLv2 数据集下，模型训练的可视化结果，如 Fig. \ref{fig: ULite_Enhance_4_layer_EM_without_Transformer} 所示。
		\end{itemize}
		...
		\begin{figure}[htbp]
			...
			\begin{subfigure}{0.45\textwidth}
				\includegraphics[width=\linewidth]{picture/LLIE/Experiment/EM/ULite_Enhance_3_layer}
				\captionsetup{font=scriptsize}
				\caption{3层结构的U-Net}
				\label{fig: ULite_Enhance_3_layer}	
			\end{subfigure}
			\begin{subfigure}{0.45\textwidth}
				\includegraphics[width=\linewidth]{picture/LLIE/Experiment/EM/ULite_Enhance_3_layer_EM}
				\captionsetup{font=scriptsize}
				\caption{添加EM模块的3层结构的U-Net}
				\label{fig: ULite_Enhance_3_layer_EM}	
			\end{subfigure}
			\begin{subfigure}{0.45\textwidth}
				\includegraphics[width=\linewidth]{picture/LLIE/Experiment/EM/ULite_Enhance_3_layer_EM_withou_Transformer_maxc_512_ReLU}
				\captionsetup{font=scriptsize}
				\caption{去除BottleNeck}
				\label{fig: ULite_Enhance_3_layer_EM_withou_Transformer_maxc_512_ReLU}	
			\end{subfigure}
			\begin{subfigure}{0.45\textwidth}
				\includegraphics[width=\linewidth]{picture/LLIE/Experiment/EM/ULite_Enhance_3_layer_EM_without_Transformer}
				\captionsetup{font=scriptsize}
				\caption{去除BottleNeck}
				\label{fig: ULite_Enhance_3_layer_EM_without_Transformer}	
			\end{subfigure}
			\begin{subfigure}{0.45\textwidth}
				\includegraphics[width=\linewidth]{picture/LLIE/Experiment/EM/ULite_Enhance_3_layer_EM_without_Transformer_maxc_512}
				\captionsetup{font=scriptsize}
				\caption{去除BottleNeck}
				\label{fig: ULite_Enhance_3_layer_EM_without_Transformer_maxc_512}	
			\end{subfigure}
			\begin{subfigure}{0.45\textwidth}
				\includegraphics[width=\linewidth]{picture/LLIE/Experiment/EM/ULite_Enhance_4_layer_EM_without_Transformer}
				\captionsetup{font=scriptsize}
				\caption{去除BottleNeck}
				\label{fig: ULite_Enhance_4_layer_EM_without_Transformer}	
			\end{subfigure}
			\caption{呈现效果}
